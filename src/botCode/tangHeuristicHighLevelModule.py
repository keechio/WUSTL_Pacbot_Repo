...
# This is awful. Look online to find out blue is supposed to move, and let's just work under the
    ...
    def _get_next_blue_chase_move(self, p_loc, red_pos, grid):
        pacbot_target = (0, 0)
        if self._get_direction(p_loc) == up:
            pacbot_target = (
                p_loc[0] - 2, p_loc[1] + 2)
        elif self._get_direction(p_loc) == down:
            pacbot_target = (
                p_loc[0], p_loc[1] - 2)
        elif self._get_direction(p_loc) == left:
            pacbot_target = (
                p_loc[0] - 2, p_loc[1])
        elif self._get_direction(p_loc) == right:
            pacbot_target = (
                p_loc[0] + 2, p_loc[1])
    ...
    def _get_move_based_on_target(self, target, target_prev, cur_loc, grid):
