...
commandstop = False
#definitions
cardToInt = {"N":0, "E":1, "S":2, "W":3}
basePower = 0.2

while commandstop == False:
    ...
    range4 = dis4.range # right sensors
    range5 = dis5.range # 
    
    delta = 0.03
    motor.tank(basePower+delta*gyro_angle,basePower - delta*gyro_angle)
    print('angle ' + str(gyro_angle))

    if range4 < 60 :
        motor.stop()
        motor.drive(basePower, 0, direction=False)
        # motor.spin_left(0.1, 5)
        time.sleep(0.1)
        motor.drive(basePower, basePower)
        time.sleep(0.1)
        gyro.reset()
        gyro_angle = 0
        ...
        motor.stop()
        motor.drive(0, basePower, direction=False)
